...
describe('Expression Rewriting', function() {
    ...
    it('Should be able to parse and evaluate object literals containing division', function() {
        var result = ko.expressionRewriting.parseObjectLiteral("a: null/1, b: 2/1, c: (6) / 2");
        expect(result).toEqual([{key:'a', value: 'null/1'}, {key: 'b', value: '2/1'}, {key: 'c', value: '(6)/2'}]);
        var rewritten = ko.expressionRewriting.preProcessBindings(result, {valueAccessors:true});
        expect(rewritten).toEqual("'a':function(){return null/1},'b':function(){return 2/1},'c':function(){return (6)/2}");
        var evaluated = eval("({" + rewritten + "})");
